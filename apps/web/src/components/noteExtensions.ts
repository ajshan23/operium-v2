...
// Mermaid source is rendered by Mermaid, not auto-detected as a programming
// language. Avoid expensive highlightAuto passes over large diagram blocks.
lowlight.register('mermaid', () => ({ name: 'Mermaid', contains: [] }));
lowlight.registerAlias('mermaid', ['Mermaid']);
...
export function noteExtensions({ editable }: { editable: boolean }) {
  return [
    ...
    Link.configure({
      ...
      HTMLAttributes: {
        class: 'text-accent-text underline cursor-pointer hover:opacity-80 transition-colors',
...
export const NOTE_PROSE_CLASS = `note-prose prose max-w-none outline-none
  ...
  [&_blockquote]:border-l-2 [&_blockquote]:border-accent [&_blockquote]:pl-3 [&_blockquote]:py-1 [&_blockquote]:my-2 [&_blockquote]:italic [&_blockquote]:text-content-secondary [&_blockquote]:bg-surface-raised [&_blockquote]:rounded-r-md
  ...
  [&_li]:pl-1 [&_li>p]:m-0
  [&_pre]:bg-surface-page [&_pre]:p-3 [&_pre]:rounded-lg [&_pre]:border [&_pre]:border-line-subtle [&_pre]:text-content-secondary [&_pre]:font-mono [&_pre]:text-xs [&_pre]:leading-[1.6] [&_pre]:my-2 [&_pre]:overflow-x-auto
  [&_pre_code]:bg-transparent [&_pre_code]:p-0 [&_pre_code]:border-0 [&_pre_code]:text-inherit
  [&_code]:font-mono
  [&_:not(pre)>code]:px-1 [&_:not(pre)>code]:py-0.5 [&_:not(pre)>code]:rounded [&_:not(pre)>code]:bg-surface-raised [&_:not(pre)>code]:text-accent-text [&_:not(pre)>code]:text-[0.9em] [&_:not(pre)>code]:border [&_:not(pre)>code]:border-line
  ...
  [&_th]:bg-surface-raised [&_th]:border-b [&_th]:border-line-subtle [&_th]:border-r [&_th]:border-line-subtle [&_th]:px-3 [&_th]:py-2 [&_th]:font-bold [&_th]:text-content-primary [&_th]:text-left
