...
import { personalTaskScope } from "@operium/core";
...
export class TasksService {
  private async assertAssigneeInOrg(orgId: string | undefined, assigneeId: string, userId: string) {
    if (assigneeId === userId) return;
    if (!orgId) throw new ApiError(400, "An organization is required to assign a task to someone else");
  ...
  async list(userId: string, orgId: string | undefined, status?: string) {
    const filter: any = personalTaskScope(userId, orgId);
  ...
  async create(userId: string, orgId: string | undefined, data: CreateTaskData) {
  ...
  async update(id: string, userId: string, orgId: string | undefined, data: UpdateTaskData) {
    const upd: any = {};
    ...
    const task = await Task.findOneAndUpdate(
      { _id: id, ...personalTaskScope(userId, orgId) },
      upd,
  ...
  async delete(id: string, userId: string, orgId: string | undefined) {
    const task = await Task.findOneAndDelete({ _id: id, ...personalTaskScope(userId, orgId) });
    if (!task) throw new ApiError(404, "Task not found");
    return { deleted: true };
  }

  async stats(userId: string, orgId: string | undefined) {
    const tasks = await Task.find(personalTaskScope(userId, orgId)).select("status").lean();
